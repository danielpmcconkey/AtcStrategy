...
| 12 | "Fix loop creates divergence" — iterating fixes might drift from original requirements | C-09 | **Resolution documentation** | Every fix attempt logged in resolution_log.md with hypothesis, evidence, and outcome. Resolution agents must check prior hypotheses before attempting a fix. BRD updates require re-citing evidence. |
| 13 | "Governance artifacts are AI-generated boilerplate" — no assurance they reflect reality | C-30 | **Proofmark config echo** | Governance reports include the actual Proofmark config used (columns EXCLUDED/FUZZY and why), the actual comparison result, and the actual fix history. Machine-verifiable, not just narrative. |
...
| "It produces governance artifacts" | Executive summary, per-job reports with Proofmark config echo |
